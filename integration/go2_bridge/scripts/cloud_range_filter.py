...
1. livox 插件把未击中射线的 range 置 0 后仍按 range*axis 发布 → 这些点精确落在
   传感器原点（间歇性退化期甚至整帧塌成贴脸团）；
2. MID360 装在 Go2 顶部 0.46m，正常扫描也会持续打到自己身体。
这两类点进入下游会把机器人周围格子染成占据：
- octomap 投影带 occupancy_min_z=0.5 按【体素】过滤，z=0.46 的点落在 [0.4,0.8)
  体素层、体素中心 0.6>0.5 照样通过；
- SCAN-Planner grid_map 完全没有近距过滤。
地图被污染后 ARiADNE 与 SCAN 对同一格子判断相反 → 目标被拒 → 狗站桩死锁。
