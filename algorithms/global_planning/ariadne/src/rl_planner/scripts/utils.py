...
def is_free_with_margin(location, map_info, margin=1):
    # 四足适配（go2-scan）：目标格及其 margin 圈邻域全 free 才算安全。
    # 背景：ARiADNE 的 0.4m 投影图分辨率粗，节点可能落在离墙脸仅 0.1~0.3m 的格子里
    #（本格 free、邻格全是墙），下游 SCAN-Planner 精细地图+膨胀必拒收 → 站桩死锁。
    ...
    for dy in range(-margin, margin + 1):
        for dx in range(-margin, margin + 1):
            ...
            if not (0 <= x < w and 0 <= y < h) or map_info.map[y, x] != parameter.FREE:
                return False
    return True


def find_approach_point(start, goal, map_info, step=None):
    # 四足适配（go2-scan）：目标太贴障碍时，沿 start→goal 直线找最后一个
    # 「带 margin free」的采样点作为逼近点（狗贴近障碍物，雷达才能看清绕行通道）。
    if step is None:
        step = map_info.cell_size / 2
    ...
    while d < length:
        p = start + direction * d
        if not is_free_with_margin(p, map_info):
            break
        best = np.around(p, 1)
        d += step
    return best
...
def check_collision_ignore_unknown(start, end, map_info):
    # 四足适配（go2-scan）2026-08-24：与 check_collision 同一 Bresenham 骨架，唯一区别是
    # UNKNOWN 格不视为阻挡（OCCUPIED 仍然阻挡）。用途：效用归因（节点能看到哪些前沿）。
    # 背景：真实传感器投影下，门洞前沿只有 1~2 格、紧邻门框锯齿格，unknown-阻挡视线
    # 会把全部门洞前沿判为不可见 → 全图 utility 恒零 → 33 秒即误判"探索完成"
    # （run2/run4 实录；离线复现：46 对视线 46 挡）。上游完美传感器无此问题。
    # 只影响"能见度评估"，路径有效性检查仍用原版 check_collision。
    collision = False
