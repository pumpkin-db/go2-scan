...
{
const std::string kWorldFrameID = "world";  // 改 world 与 Gazebo 仿真一致（原为 "map"）
...
private:
  ...
  ros::Publisher initial_path_pub_;  // 全局决策路径输出，接 SCAN-Planner /initial_path
  ...
  void PublishInitialPath(const exploration_path_ns::ExplorationPath& global_path);
