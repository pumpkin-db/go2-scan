...
# 复核：还有任何残留则再杀一轮
left=$(pgrep -f "gzserver|rosmaster|rosout|roslaunch|scan_planner_node|go2_kinematic_sim|octomap_server|sensorScanGeneration|rl_planner.py|ariadne_goal_bridge.py" 2>/dev/null)
if [ -n "$left" ]; then
