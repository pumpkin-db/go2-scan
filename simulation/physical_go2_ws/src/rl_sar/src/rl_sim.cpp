...
RL_Sim::RL_Sim(int argc, char **argv)
{
    ...
    private_nh.param<bool>("auto_getup", this->auto_getup, false);
    private_nh.param<double>("auto_getup_settle_seconds", this->auto_getup_settle_seconds, 1.0);
    private_nh.param<double>("auto_getup_max_body_z", this->auto_getup_max_body_z, 0.20);
    ...
    for (int i = 0; i < this->params.Get<int>("num_of_dofs"); ++i)
    {
        ...
        this->joint_state_received[joint_controller_name] = false;
    }
...
{
#if defined(USE_ROS1)
    if (this->auto_getup && !this->auto_getup_sent)
    {
        const bool all_joints_ready = std::all_of(
            this->joint_state_received.begin(), this->joint_state_received.end(),
            [](const auto &entry) { return entry.second; });
        const bool body_settled = this->model_state_received &&
            this->pose.position.z <= this->auto_getup_max_body_z &&
            std::abs(this->vel.linear.z) <= 0.05;

        if (all_joints_ready && body_settled)
        {
            const auto now = std::chrono::steady_clock::now();
            if (this->auto_getup_ready_since.time_since_epoch().count() == 0)
            {
                this->auto_getup_ready_since = now;
                ROS_INFO("[rl_sim] Physical Go2 is settled; arming automatic GetUp");
            }
            const double ready_seconds = std::chrono::duration<double>(
                now - this->auto_getup_ready_since).count();
            if (ready_seconds >= this->auto_getup_settle_seconds)
            {
                this->control.SetKeyboard(Input::Keyboard::Num0);
                this->auto_getup_sent = true;
                ROS_INFO("[rl_sim] Automatic GetUp triggered (keyboard 0 equivalent)");
            }
        }
        else
        {
            this->auto_getup_ready_since = std::chrono::steady_clock::time_point{};
        }
    }
#endif
...
void RL_Sim::ModelStatesCallback(const gazebo_msgs::ModelStates::ConstPtr &msg)
{
    ...
    {
        ROS_WARN_THROTTLE(2.0, "[rl_sim] Waiting for Gazebo model '%s'", this->gazebo_model_name.c_str());
        return;
    }

    const auto model_index = static_cast<std::size_t>(std::distance(msg->name.begin(), model_it));
    if (model_index >= msg->pose.size() || model_index >= msg->twist.size())
    {
        ROS_ERROR_THROTTLE(2.0, "[rl_sim] Invalid /gazebo/model_states array sizes for '%s'", this->gazebo_model_name.c_str());
        return;
    }

    this->vel = msg->twist[model_index];
    this->pose = msg->pose[model_index];
    this->model_state_received = true;
}
#elif defined(USE_ROS2)
...
void RL_Sim::JointStatesCallback(const robot_msgs::MotorState::ConstPtr &msg, const std::string &joint_controller_name)
{
    ...
    this->joint_state_received[joint_controller_name] = true;
}
#elif defined(USE_ROS2)
