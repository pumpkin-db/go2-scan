...
bool SensorCoveragePlanner3D::initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p)
{
  ...
  initial_path_pub_ = nh.advertise<nav_msgs::Path>("/initial_path", 1);  // 接 SCAN-Planner
...
void SensorCoveragePlanner3D::PublishInitialPath(const exploration_path_ns::ExplorationPath& exploration_path)
{
  // exploration_path = ConcatenateGlobalLocalPath 的结果（实际是 local_path，局部视点序列，非闭环，不截尾）
  nav_msgs::Path path = exploration_path.GetPath();
  path.header.frame_id = kWorldFrameID;
  path.header.stamp = ros::Time::now();
  initial_path_pub_.publish(path);
}
