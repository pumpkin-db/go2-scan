...
def _utility_los(start, end, map_info):
    # 效用归因（节点可见哪些前沿）专用的视线检查。LOS_IGNORE_UNKNOWN=True 时
    # unknown 格不阻挡视线（门洞前沿可被看到，防过早判完成，2026-08-24）；
    # 路径有效性检查仍用原版 check_collision，不受此开关影响。
    if getattr(parameter, 'LOS_IGNORE_UNKNOWN', False):
        return check_collision_ignore_unknown(start, end, map_info)
    return check_collision(start, end, map_info)
...
class NodeManager:
    ...
    def remove_unconnected_nodes(self, origin):
        ...
        # go2-scan 四足适配（2026-08-24）：origin(=开机起点)节点可能已被 check_valid_node
        # 当作障碍清理掉，find() 返回 None 直接崩掉 Timer 线程（run8 traceback 实锤，
        # 也是此前多轮"无声停摆"的真凶）。回退到离 origin 最近的现存节点继续连通性清理；
        # 图为空则无事可做直接返回。
        current = self.nodes_dict.find(origin.tolist())
        if current is None:
            if len(all_nodes_coords) == 0:
                return
            origin = np.array(self.nodes_dict.nearest_neighbors(origin.tolist(), 1)[0].data.coords)
            current = self.nodes_dict.find(origin.tolist())
    ...
    def initialize_observable_frontiers(self, frontiers, updating_map_info):
        if len(frontiers) == 0:
            ...
            for point in new_frontiers_in_range:
                collision = _utility_los(self.coords, point, updating_map_info)
                if not collision:
